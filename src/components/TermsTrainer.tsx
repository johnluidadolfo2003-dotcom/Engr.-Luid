...
export function TermsTrainer() {
  ...
  return <div className="terms-page">
    <header className="terms-hero"><div><span className="review-eyebrow">TERM PRACTICE</span><h1>Learn the terms.</h1></div><div className="terms-stat"><strong>{mastered}<small>/{termBank.length}</small></strong><span>terms learned</span><div className="review-meter"><i style={{width:`${100*mastered/termBank.length}%`}}/></div></div></header>
    ...
    {mode==='drill' ? card ? <div className="terms-layout terms-layout-simple"><section className="term-question"><div className="term-meta"><span>{card.track==='electrical'?'ELECTRICAL':card.track==='mathematics'?'MATHEMATICS':'ESAS'}</span><span>{topic?.title}</span></div><h2>Name the term</h2><p className="term-definition">{card.meaning}</p><div className="term-options">{options.map((option,i)=><button key={option.id} disabled={answer!==null} onClick={()=>grade(option.id)} className={answer===null?'':option.id===card.id?'correct':answer===option.id?'incorrect':'muted'}><span>{String.fromCharCode(65+i)}</span>{option.term}</button>)}</div>
      ...
      </section></div> : <div className="terms-empty"><BookOpen size={30}/><h2>Review queue finished.</h2><p>Come back tomorrow or practice all terms now.</p><button onClick={()=>{setReviewAll(true);resetSelection()}}>Practice all terms</button></div>
      : <div className="term-library"><label className="review-search"><Search size={16}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search a term or definition" aria-label="Search terms"/></label><p>{filtered.length} terms</p><div>{filtered.map(t=>{const s=officialCoverage.find(x=>x.track===t.track);return <article key={t.id}><small>{s?.topics.find(x=>x.id===t.group)?.title}</small><h3>{t.term}</h3><p>{t.meaning}</p><span>{t.distinguish}</span></article>})}</div></div>}
  </div>;
}
