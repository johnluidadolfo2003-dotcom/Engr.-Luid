...
export const FormulaVault:React.FC<Props> = () => {
  ...
  return <div className="formula-page"><header className="formula-hero"><span className="review-eyebrow">QUICK REFERENCE</span><h1>See the relationship.</h1><p>Find a formula, read the symbols, and connect it to the physical idea. Work a lesson example before memorizing it.</p></header>
    ...
    <div className="formula-result-count">{filtered.length} formulas</div><div className="formula-grid">{filtered.map(f=><article key={f.id} className="formula-tile"><header><div><small>{f.subject} / {f.category}</small><h2>{f.title}</h2></div><button onClick={()=>copy(f)} title="Copy formula" aria-label={`Copy ${f.title}`}>{copied===f.id?<Check size={16}/>:<Copy size={16}/>}</button></header><div className="formula-display" aria-label={`Formula: ${formatEngineeringFormula(f.formula)}`}><span>RELATIONSHIP</span><strong>{formatEngineeringFormula(f.formula)}</strong></div><div className="formula-symbols"><span>SYMBOLS & UNITS</span><p>{f.variables}</p><small>{f.units}</small></div><details><summary>When to use it <ChevronDown size={14}/></summary><p>{f.plainEnglish}</p><p>{f.boardExamTip}</p></details></article>)}</div>{!filtered.length&&<div className="terms-empty"><h2>No matching formulas.</h2><p>Try another subject or a shorter search term.</p></div>}</div>;
};
