...
export const CurriculumStudyHub: React.FC<Props> = ({ onAskMentor, onOpenGrapher }) => {
  ...
  const chooseCoverageTopic = (targetTrack: CurriculumTrack, lessonId: string) => {
    setTrack(targetTrack); setSelectedId(lessonId); setSearch(''); setAnswers({}); setRevealed(false);
  ...
  return <div ref={topRef} className="review-page">
    <CoverageMap active={track} onChoose={chooseCoverageTopic} completed={completed}/>
    <section className="review-hero">
      <div><div className="review-eyebrow">REE REVIEW</div><h1>See it. Solve it.<br/><em>Recall it.</em></h1></div>
    ...
    <div className="review-layout">
      ...
      <article ref={articleRef} className="review-article" key={selected.id}>
        <div className="lesson-header"><div className="lesson-kicker"><span>LESSON {selected.lessonNumber}</span><span>·</span><span>{selected.stageTitle.replace(/^Stage \d+: /, '').replace(/^\d+\. /, '')}</span></div><h2>{selected.title}</h2><div className="lesson-actions"><button onClick={markDone} className={completed.has(selected.id) ? 'completed' : ''}><Check size={16}/>{completed.has(selected.id) ? 'Completed' : 'Mark complete'}</button><button onClick={ask}><Sparkles size={16}/> Ask</button></div></div>
        <div className="lesson-content">
          <LessonVisual lesson={selected}/>
          <section className="lesson-section"><div className="section-label"><span>01 / THE IDEA</span></div><div className="idea-grid"><div className="idea-main"><p>{compact(selected.coreTheory[0] || selected.summary, 260)}</p></div><div className="idea-memory"><span>REMEMBER THIS</span><strong>{compact(selected.boardExamTip, 125)}</strong></div></div>{selected.coreTheory.length > 1 && <details className="more-detail"><summary>More detail <ChevronDown size={15}/></summary><ul>{selected.coreTheory.slice(1).map((line,i)=><li key={i}>{line}</li>)}</ul></details>}</section>
          ...
          {selected.formulas.length > 0 && <section className="lesson-section"><div className="section-label"><span>02 / THE RELATIONSHIP</span></div><div className="formula-stack">{selected.formulas.map((f,i)=><div className="formula-card" key={i}><div><span>{f.name}</span><small>{f.variables}</small></div><strong className="math-expression">{formatEngineeringFormula(f.formula)}</strong></div>)}</div></section>}
          <section className="lesson-section"><div className="section-label"><span>{selected.formulas.length ? "03" : "02"} / WORK ONE OUT</span></div><div className="worked-card"><h3>{compact(selected.workedExample.problem, 260)}</h3><div className="given-row">{selected.workedExample.given.map((g,i)=><span key={i}>{g}</span>)}</div><ol>{selected.workedExample.stepByStep.map((s,i)=><li key={i}><span>{String(i+1).padStart(2,'0')}</span><p>{s.replace(/^Step \d+\s*(\([^)]*\))?\s*:\s*/, '')}</p></li>)}</ol><div className="worked-answer"><span>RESULT</span><strong>{selected.workedExample.answer}</strong></div></div></section>
          <section ref={practiceRef} className="lesson-section"><div className="section-label"><span>{selected.formulas.length ? "04" : "03"} / RECALL</span></div>{questions.map(q => <div className="recall-card" key={q.id}><h3>{q.question}</h3><div className="recall-options">{q.options.map((option,i)=><button key={i} disabled={answers[q.id] !== undefined} onClick={() => setAnswers(prev => ({...prev,[q.id]:i}))} className={answers[q.id] === undefined ? '' : i === q.correctAnswer ? 'correct' : answers[q.id] === i ? 'incorrect' : 'muted'}><span>{String.fromCharCode(65+i)}</span>{option}</button>)}</div>{answers[q.id] !== undefined && <div className="recall-feedback"><strong>{answers[q.id] === q.correctAnswer ? 'Correct.' : 'Review this.'}</strong><p>{compact(q.explanation, 250)}</p></div>}</div>)}{!questions.length && <p>Review the example, then explain the relationship aloud in one sentence.</p>}{allCorrect && !completed.has(selected.id) && <button className="review-primary" onClick={markDone}><Check size={16}/> Save as completed</button>}</section>
        </div>
